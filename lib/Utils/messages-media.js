...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (filePath, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        // Transform media path for newsletter uploads
        if (newsletter) {
        ...
        for (const { hostname } of hosts) {
            ...
            try {
                ...
                if (result?.url || result?.direct_path) {
                    logger.debug({ result }, 'Upload response from WhatsApp')
                    // Handle extraction priority:
                    // 1. result.handle (direct from WhatsApp, especially for newsletters)
                    // 2. result.fbid (standard for regular uploads)
                    // 3. result.file_id (alternative)
                    // 4. Extract from direct_path (fallback)
                    const handle = result.handle || result.fbid || result.file_id || result.direct_path?.split('?')[0]?.split('/').pop() || result.direct_path
                    ...
                    logger.debug({ urls, handle }, 'Constructed URLs object with handle')
                    break
                }
                else {
