        ...
        for (const { hostname } of hosts) {
            ...
            try {
                ...
                if (result?.url || result?.direct_path) {
                    ...
                    // For newsletter uploads, fbid might not be returned
                    // Try fbid first, then file_id, then extract from direct_path
                    const handle = result.fbid || result.file_id || result.direct_path?.split('?')[0]?.split('/').pop() || result.direct_path
